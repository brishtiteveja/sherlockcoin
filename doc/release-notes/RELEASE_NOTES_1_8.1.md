# sherlockcoin Core 1.8.1

sherlockcoin Core 1.8.1 is primarily a bugfix release, bringing sherlockcoin Core in
line with Bitcoin 0.9.3. sherlockcoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by sherlockcoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 sherlockcoin Core clients
* Use HTTPS with shcchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/sherlockcoin/sherlockcoin/issues/134).
...
This only affects values significantly below 1 shc, however merchants may wish to round to
the nearest shc on all transactions for now.
