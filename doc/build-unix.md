UNIX BUILD NOTES
====================
Some notes on how to build sherlockcoin in Unix. 
...
This will build sherlockcoin-qt as well if the dependencies are met.
...
memory available when compiling sherlockcoin Core. With 512MB of memory or less
...
If you want to build sherlockcoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a sherlockcoin-qt executable will be
built by default.

Notes
-----
The release is built with GCC and then "strip sherlockcoind" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the sherlockcoin directory
...
# Configure sherlockcoin Core to use our own-built instance of BDB
cd $BITCOIN_ROOT
...
cryptography functions, and therefore cannot be used for sherlockcoin (or other
...
Security
--------
To help make your sherlockcoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, sherlockcoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, sherlockcoin may be compiled in
